...
[2026-06-08] [Claude_up] MCP 注册机制纠偏（晚于上一条提交发现）：Claude Code v2.1.x **不读** `~/.claude/settings.json` 的 mcpServers，必须 `claude mcp add -s user` 写入 `~/.claude.json`。已迁 cognee + filesystem/playwright/context7（github 待 GITHUB_TOKEN）；cognee 用 `09-cognee/cognee-mcp-wrapper.sh` 从环境读 key，~/.claude.json env `{}` 零硬编码 · 原因：实测 settings.json 里 5 个 server 一个都没加载（`claude mcp list` 仅 feishu），推翻旧 L003，详见 L024
[2026-06-08] [Claude_up] cognee 跨工具共享：Codex CLI 经 `~/.codex/config.toml` 的 `[mcp_servers.cognee]` 指向同一 wrapper，与 Claude 共用同一知识图谱（09-cognee/.data_storage+.cognee_system）；经典 Desktop 纯聊天加到 claude_desktop_config.json mcpServers · 原因：cognee 工具无关，一套记忆多工具复用，契合"Skill 跨工具通用"理念
[2026-06-08] [Claude_up] cognee 并发根治（方案 C：单 API server + 瘦客户端）：常驻 1 个 cognee FastAPI server（launchd `com.moonz.cognee-api`，绑 127.0.0.1:8000，`REQUIRE_AUTHENTICATION=false` 无 token）独占文件库；3 个 host 经 wrapper `--api-url` 委托 · 原因：kuzu 单写锁致 Claude/Codex/Desktop 撞锁（L025），API server 方案保留数据零迁移、比 Neo4j 轻，实测 3 并发 0 锁错（详见 L026）；旧 P002"避免常驻服务"在 Mac(CLI 可达 localhost)下已不构成阻碍
